...
The Function emits a privacy-safe structured log for each accepted event. For durable aggregate reporting, add a Cloudflare Pages Analytics Engine binding named `CONVERSION_EVENTS` and point it to a dataset named `digitrust_conversion_events`. Configure this in the Pages project dashboard rather than adding a new Wrangler file, because the existing Pages project settings remain the deployment source of truth.
